...
def _main(args, output_file):
    ...
    generator = task.build_generator(forward_args)
    ...
    if args.prefix_inference == 'bidi':
        ...
        model_dict['bwd_prefix'] = DirectionTask(
            bwd_prefix_task, bwd_prefix_model, bwd_prefix_task.build_generator(bwd_prefix_args))
    else:
    ...
    if args.sequence_inference == 'bidi':
        ...
        model_dict['bwd_seq'] = DirectionTask(
            bwd_sequence_task, bwd_sequence_model, bwd_sequence_task.build_generator(bwd_sequence_args))
    else:
    ...
    if args.forward_prefix_path:
        ...
        model_dict['fwd_prefix'] = DirectionTask(
            fwd_prefix_task, fwd_prefix_model, fwd_prefix_task.build_generator(fwd_prefix_args))
    else:
        logger.info("no forward prefix model; using full forward model for prefix inference")
        fwd_prefix_model = None
        model_dict['fwd_prefix'] = DirectionTask(task, models[0], task.build_generator(args))
...
def cli_main():
    ...
    if args.vanilla:
        from fairseq_cli.vanilla_generate import main as vanilla_main
        vanilla_main(args)
    else:
        main(args)
